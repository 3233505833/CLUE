...
def run():
    ...
    for stage in stages:
        ...
        if "2_" + str(stage) in step_idx_lst or "2" in step_idx_lst:
            import os
            rater = "a"
            step1_save_path = os.path.join(gpt3_for_text_cls_task.config.save_log_dir,
                                           "query" + str(int(stage)) + "_input.jsonl")
            gpt3_for_text_cls_task.run_query_pre_results(
                stage, rater, step1_save_path, True if args.resume else False
            )
            if stage != "4":
                refer_doc_path = os.path.join(gpt3_for_text_cls_task.config.save_log_dir,
                                              "unjudge" + str(int(stage) + 1) + ".jsonl")
            else:
                refer_doc_path = os.path.join(gpt3_for_text_cls_task.config.save_log_dir, f"doc_data.jsonl")
            output_query_path = os.path.join(gpt3_for_text_cls_task.config.save_log_dir,
                                             f"query{int(stage)}{rater}_output.jsonl")
            output_update_query_path = os.path.join(gpt3_for_text_cls_task.config.save_log_dir,
                                                    f"query{int(stage)}{rater}_update_output.jsonl")
            saved_result_doc_path = os.path.join(gpt3_for_text_cls_task.config.save_log_dir,
                                                 f"judge{int(stage)}{rater}.jsonl")
            gpt3_for_text_cls_task.query_jsonal_doc(refer_doc_path, output_query_path, output_update_query_path,
                                                    saved_result_doc_path, stage)
            file_list = [os.path.join(gpt3_for_text_cls_task.config.save_log_dir,
                                      f"judge" + str(int(stage)) + rater + ".jsonl") for rater in ["a"]]
            judge_pre_file = os.path.join(gpt3_for_text_cls_task.config.save_log_dir,
                                          f"judge" + str(int(stage)) + "_pre.jsonl")
            gpt3_for_text_cls_task.merge_jsonl_file(file_list, judge_pre_file)
            from utils.rater_group import group_1_rater
            judge_pre_file = os.path.join(gpt3_for_text_cls_task.config.save_log_dir,
                                          f"judge" + str(int(stage)) + "_pre.jsonl")
            judge_file = os.path.join(gpt3_for_text_cls_task.config.save_log_dir, f"judge" + str(int(stage)) + ".jsonl")
            group_1_rater(judge_pre_file, judge_file, stage)
