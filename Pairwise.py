...
class GPT3TextCLS(object):
    ...
    def doc_to_query_jsonl(self, query_save_path: str, step1_prompt_data_path: str, stage: str):
        ...
        for key_value, batch_data_item in tqdm(batches.items(), total=len(batches), desc="Processing Batches"):
            for i in range(len(batch_data_item) - 1):
                for j in range(i + 1, len(batch_data_item)):
                    if self.config["data_name"] == "KDD":
                        doc_i = query_level_feature(batch_data_item[i], self.df)
                        doc_j = query_level_feature(batch_data_item[j], self.df)
                    else:
                        doc_i = format_document_new(batch_data_item[i])
                        doc_j = format_document_new(batch_data_item[j])
                    pairwise_prompts = [
                        {"role": "system",
                         "content": "你是信息检索领域的智能助手，能够根据信息搜索者的信息需求和查询对文档的有用性进行评分。"}
                    ]
                    pairwise_prompts.append({
                        "role": "user",
                        "content": f"现在的信息需求和查询是: {batch_data_item[0]['prompt_text']}。我会提供给你2个文档，每个文档都使用一个[数字]进行标识。"
                    })
                    pairwise_prompts.append({'role': 'assistant', 'content': '好的，请提供文档。'})

                    # 构建pairwise prompt
                    prompt = (
                        f"{doc_i}\n"
                        f"{doc_j}\n"
                        "请判断哪个文档的有用性更高，输出格式应为“[文档编号]>[另一个文档编号]”代表你的选择。请严格按照这个格式输出，不要输出任何其他的字符或者汉字。"
                    )
                    pairwise_prompts.append({
                        "role": "user",
                        "content": prompt
